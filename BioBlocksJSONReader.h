...
{
public:
    typedef std::unordered_multimap<std::tuple<long,long>, BioBlocks::PipetteOperation, PairLongLongHashFunction> OperationsMultiMap;
...
protected:
    ...
    void readInstructions(const nlohmann::json & js, OperationsMultiMap & operationHeap);

    void parseHeap(OperationsMultiMap & operationHeap, ProtocolGraph* protocol);
    void parseHeap_processOpBlock (ProtocolGraph* protocol,
                         ...
                         int & lastInitTime,
                         int & lastDuration);
    void parseHeap_createInternalVariables(ProtocolGraph* protocol, OperationsMultiMap & operationHeap);

    void parsePipette(const nlohmann::json::iterator & it, OperationsMultiMap & operationHeap);
    void parsePipette_one2many(const std::string & key, nlohmann::json to, nlohmann::json from, float timeInit, float duration, OperationsMultiMap & operationHeap);
    void parsePipette_many2one(const std::string & key, nlohmann::json to, nlohmann::json from, nlohmann::json value, float timeInit, float duration, OperationsMultiMap & operationHeap);
    void parsePipette_one2one(const std::string & key, nlohmann::json to, nlohmann::json from, nlohmann::json value, float timeInit, float duration, OperationsMultiMap & operationHeap);
